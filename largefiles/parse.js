...
function parse(slicer, bytes) {
  ...
  if (_MODE_ == 'header') {
    ...
    if (_next_ifd == 0) {
      ...
      volume._max = 100;
      volume._windowLow = 0;
      volume._windowHigh = 100;
      volume.lowerThreshold = 0;
      volume.upperThreshold = 100;
      volume.create_();
    ...
    s.setup([0, 0, _CURRENT_SLICE - _TIFF_.ifd.length / 2], [0, 0, 1],
        [0, 1, 0], _CURRENT_IFD['IMAGE_WIDTH'], _CURRENT_IFD['IMAGE_LENGTH'],
        false);
    ...
    _LOADED_SLICES.push(_CURRENT_SLICE);
...
function process_queue() {
  ...
  show(_QUEUE.shift());
  console.log('Cached:', _LOADED_SLICES);
  console.log('Queued:', _QUEUE);
...
function reset_cache() {
  ...
  var i;
  for (i = 0; i < _CACHE_SIZE - _CACHE_LIMIT; i++) {
    
    // remove slice from renderer
    var _slice = volume.children[2].children[_LOADED_SLICES[i]];
    
    // remove slice from volume
    ren3d.remove(_slice);
...
function onKey(e) {
  ...
  var _current_slice = volume.indexZ;
  console.log('navigating to ', volume.indexZ);
  
  var _cache_position = _LOADED_SLICES.indexOf(volume.indexZ);
  
  console.log('scrolling..', _cache_position, ' of ', _LOADED_SLICES.length -
      2 * _CACHE_LIMIT);
  
  if (_cache_position > _LOADED_SLICES.length - 2 * _CACHE_LIMIT) {
...
function addUI() {
  ...
  var _sliceController = gui.add(volume, 'indexZ', 0, volume.dimensions[2] - 1,
      1);
  
  _sliceController.onChange(function(v) {
